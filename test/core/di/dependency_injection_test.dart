...
void main() {
  ...
  // driftDatabase resolves its storage path via path_provider, which requires
  // a native plugin. In unit tests, we stub the channel to return the system
  // temp directory so no platform embedding is needed.
  ...
  group('setupDependencies — registration', () {
    ...
    test('registers CharacterApiService', () => expect(getIt.isRegistered<CharacterApiService>(), isTrue));
    test('registers CharacterLocalStorage', () => expect(getIt.isRegistered<CharacterLocalStorage>(), isTrue));
    test('registers CharacterRepository', () => expect(getIt.isRegistered<CharacterRepository>(), isTrue));
    test('registers CharacterToProvidersMapper', () => expect(getIt.isRegistered<CharacterToProvidersMapper>(), isTrue));
    test('registers GetCharactersUseCase', () => expect(getIt.isRegistered<GetCharactersUseCase>(), isTrue));
    test('registers ResetCharacterStatsUseCase', () => expect(getIt.isRegistered<ResetCharacterStatsUseCase>(), isTrue));
  });
